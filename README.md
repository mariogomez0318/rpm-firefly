...
This project presents an embedded control implementation for a DC motor system integrating real-time speed measurement, temperature monitoring, and serial data acquisition within a PIC24F microcontroller architecture. The framework employs a PWM-based actuation scheme using Output Compare modules, coupled with an analog feedback channel for potentiometer-driven reference input. Motor rotational velocity is computed through a high-resolution incremental encoder interfaced via hardware interrupts, while a 100-ms sampling interval—maintained through Timer1—enables robust estimation of shaft speed in revolutions per minute.
